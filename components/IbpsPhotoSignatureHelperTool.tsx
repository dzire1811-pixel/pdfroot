...
export function IbpsPhotoSignatureHelperTool() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => event.key === "Escape" && closeSettingsDrawer();
    const onResize = () => window.innerWidth >= 640 && closeSettingsDrawer();
    ...
    const onTouchMove = (event: globalThis.TouchEvent) => event.touches[0] && updateSettingsDrawerDrag(event.touches[0].clientY);
  ...
  function renderTypeSelector(placement: "top" | "sticky" | "drawer" = "top") {
        ...
        {DOCUMENT_ORDER.map((type) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                selectType(type);
                if (isDrawer) closeSettingsDrawer();
              }}
              className={`flex shrink-0 items-center text-left transition ${
                isSticky ? "min-h-14 min-w-[9.5rem] gap-2 rounded-xl border px-3 py-2" : isDrawer ? "min-h-16 min-w-0 gap-2 rounded-xl border px-2.5 py-2" : "min-h-20 gap-3 rounded-2xl border px-4 py-3"
              } ${
            ...
            >
              <span className={`grid shrink-0 place-items-center rounded-xl ${isSticky ? "h-10 w-10" : isDrawer ? "h-9 w-9" : "h-11 w-11"} ${isSelected ? "bg-[#FF2D2D] text-white" : "bg-slate-100 text-slate-600"}`}>
  ...
  function renderActionButtons() {
    return (
      <div data-ibps-sticky-actions="true" className="flex w-full min-w-0 items-center gap-2 sm:grid sm:grid-cols-[3.5rem_minmax(12rem,1fr)_auto] lg:w-auto lg:min-w-[30rem]">
        {renderAddButton()}
        <button type="button" onClick={() => void processDocument()} className="inline-flex min-h-12 min-w-0 flex-1 items-center justify-center gap-1 whitespace-nowrap rounded-xl bg-[#FF2D2D] px-2 py-3 text-sm font-black text-white shadow-[0_16px_35px_rgba(255,45,45,0.24)] transition hover:-translate-y-0.5 hover:bg-red-600 sm:min-h-14 sm:gap-2 sm:px-5 sm:text-base">
          <span className="sm:hidden">Resize Now</span>
          <span className="hidden sm:inline">{config.actionLabel}</span>
          <RefreshCw className="h-5 w-5" aria-hidden="true" />
        </button>
        <button type="button" onClick={resetTool} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-xl border border-slate-200 bg-white px-2 py-3 text-xs font-black text-slate-800 transition hover:border-red-200 hover:text-[#FF2D2D] sm:min-h-14 sm:gap-2 sm:px-4 sm:text-sm">
          Clear all
  ...
  function beginDrawerDrag(clientY: number) {
    if (settingsDrawerClosingRef.current) return;
      ...
      <div className="fixed inset-0 z-[60] sm:hidden">
        ...
        <div id="ibps-document-mobile-settings-drawer" role="dialog" aria-modal="true" aria-label="IBPS document type" style={{ transform: `translateY(${settingsDrawerDragOffset}px)` }} className={`absolute inset-x-0 bottom-0 flex max-h-[min(68vh,34rem)] flex-col overflow-visible rounded-t-2xl border-t border-slate-200 bg-white shadow-[0_-20px_60px_rgba(15,23,42,0.18)] ${isSettingsDrawerDragging ? "" : "transition-transform duration-[240ms] ease-out"} ${isSettingsDrawerClosing ? "" : "animate-[ibpsDocumentDrawerIn_220ms_ease-out]"}`}>
          ...
          <div className="relative shrink-0 overflow-hidden rounded-t-2xl border-b border-slate-200 px-4 pb-3 pt-5">
            <p className="text-sm font-black text-slate-950">Document type</p>
            <button type="button" onClick={closeSettingsDrawer} className="absolute right-3 top-3 grid h-8 w-8 place-items-center rounded-full bg-slate-100 text-slate-600 transition hover:bg-slate-200 hover:text-slate-950 active:scale-95" aria-label="Close document type"><X className="h-4 w-4" aria-hidden="true" /></button>
          </div>
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4">{renderTypeSelector("drawer")}</div>
          {stage === "workspace" && document && <div className="shrink-0 rounded-b-2xl border-t border-slate-200 bg-white px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3">{renderActionButtons()}</div>}
        </div>
      </div>
    );
  }
      ...
      <section ref={toolSectionRef} data-v0-managed-flow="true" data-ibps-document-workspace="true" data-ibps-helper-workspace="true" id="ibps-document-resize-tool" onDragOver={onFileDragOver} onDragLeave={onFileDragLeave} onDrop={onUploadDrop} className={`mx-auto mt-6 w-full max-w-full scroll-mt-32 overflow-visible border-0 bg-transparent p-0 text-left shadow-none ${styles.workspaceSection} ${isConstrainedWorkspace ? styles.constrainedWorkspaceSection : ""}`}>
        ...
        <div ref={workspaceRef} className={`relative min-w-0 overflow-visible bg-slate-100 transition ${styles.workspaceShell} ${isDragging ? "ring-4 ring-red-100" : ""}`}>
          ...
          <div ref={actionBarRef} data-ibps-document-action-bar="true" className={`fixed bottom-0 left-0 right-0 z-50 box-border w-full max-w-full border-t border-slate-200 bg-white/95 px-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 shadow-[0_-16px_40px_rgba(15,23,42,0.08)] backdrop-blur sm:px-6 ${isConstrainedWorkspace ? styles.flowActionBar : ""}`}>
            <div className="mx-auto flex w-full min-w-0 max-w-[1600px] flex-col gap-3 xl:grid xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
              <div className="min-w-0 xl:pl-10">
                <div className="flex min-w-0 items-center justify-between gap-3 sm:hidden">
                  <p className="min-w-0 truncate text-sm font-black text-slate-950">{config.shortLabel} selected</p>
                ...
                <div className="hidden sm:block">{renderTypeSelector("sticky")}</div>
              </div>
  ...
  return (
    <section ref={toolSectionRef} data-v0-managed-flow="true" id="ibps-document-resize-tool" className="mx-auto mt-6 w-[min(calc(100vw-2rem),64rem)] max-w-full rounded-[2rem] border border-slate-200 bg-white p-4 text-left shadow-[0_24px_70px_rgba(15,23,42,0.08)] sm:w-[min(calc(100vw-3rem),64rem)] sm:p-6">
      {renderLocalPageStyles()}
      <div className="hidden sm:block">{renderTypeSelector()}</div>
      <button ref={mobileSettingsButtonRef} type="button" onClick={openSettingsDrawer} className="flex min-h-12 w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2 text-left shadow-sm transition active:scale-[0.99] sm:hidden" aria-expanded={isSettingsDrawerOpen} aria-controls="ibps-document-mobile-settings-drawer">
        <span className="flex min-w-0 items-center gap-2.5">
          <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#FF2D2D] text-white"><DocumentIcon type={selectedType} /></span>
          <span className="min-w-0"><span className="block text-xs font-bold text-slate-500">Document type</span><span className="block truncate text-sm font-black text-slate-950">{config.label}</span></span>
        </span>
        <SlidersHorizontal className="h-5 w-5 shrink-0 text-[#FF2D2D]" aria-hidden="true" />
      </button>
      {renderUploadBox()}

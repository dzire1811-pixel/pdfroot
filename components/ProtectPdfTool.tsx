...
export function ProtectPdfTool() {
  ...
  function scrollToolStageIntoView() {
    window.requestAnimationFrame(() => {
      const toolSection = document.getElementById("protect-pdf-tool");
  ...
  function removeFile(indexToRemove: number) {
    ...
    setStatus("PDF list updated. Add a password and protect PDF.");
  }
  ...
  async function handleFiles(nextFiles: FileList | File[] | null | undefined) {
    ...
    if (invalidFile) {
      setStatus(files.length > 0 ? "PDF list is ready. Add a password and protect PDF." : "Upload a PDF and add a password.");
    ...
    setWorkflowStep("settings");
    setStatus("PDFs loaded. Add a password and protect PDF.");
  ...
  async function protectPdf() {
    ...
    try {
      ...
      for (let fileIndex = 0; fileIndex < files.length; fileIndex += 1) {
        ...
        const inputPath = `/input-${fileIndex}.pdf`;
        const outputPath = `/protected-${fileIndex}.pdf`;
        ...
        setProgress(Math.min(90, 15 + Math.round(((fileIndex + 1) / files.length) * 75)));
      }

      setProgress(92);
      setStatus(files.length === 1 ? "Preparing protected PDF..." : "Preparing protected PDF ZIP...");
      const blob =
  ...
  function reorderByDragEnter(targetIndex: number) {
    ...
    setProgress(0);
    setWorkflowStep("settings");
    setStatus("Order updated. Add a password and protect PDF.");
  }
  ...
  function renderUploadBox() {
    return (
      ...
      >
        ...
        <span className="sr-only">Upload PDF files, add a password, and download protected PDFs.</span>
  ...
  function renderAddMoreButton(disabled = false) {
    return (
      <label
        htmlFor="protect-pdf-workspace-upload"
    ...
    return (
      <div className="grid justify-items-center px-2 py-2 transition sm:px-4 sm:py-3">
        <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm sm:p-8">
          ...
          <h3 className="mt-5 text-2xl font-black tracking-tight text-slate-950">Protecting PDF...</h3>
